...
export default function Tasks() {
  ...
  const filteredTasks = tasks.filter(task => {
    ...
    const methodMatch = methodFilter === 'Tất cả' || task.method === methodFilter;
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">Theo dõi và quản lý các task nghiên cứu lai tạo và kết quả thí nghiệm</p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-6">
          <div className="rounded-lg border border-gray-200 bg-white px-6 py-4 flex flex-col justify-between min-w-[180px]">
            <span className="text-sm text-gray-600 mb-1">Task đang thực hiện</span>
            <span className="text-2xl font-semibold text-green-700">12</span>
            <span className="text-xs text-green-600 mt-1">+2 task mới</span>
          </div>
          <div className="rounded-lg border border-gray-200 bg-white px-6 py-4 flex flex-col justify-between min-w-[180px]">
            <span className="text-sm text-gray-600 mb-1">Nhật ký thí nghiệm</span>
        ...
        <div className="bg-white rounded-lg shadow-sm border p-4 flex justify-between items-center">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-700">Trạng thái task:</span>
              <select 
                value={statusFilter}
        ...
        <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                <th className="text-left p-4 font-medium text-gray-900">Tên task</th>
            ...
            <tbody>
              {currentTasks.map((task) => (
                <tr key={task.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-4">
                    <div className="flex gap-2">
                      <button onClick={() => void navigate(`/tasks/${task.id}`)} className="text-blue-600 hover:bg-blue-50 p-1 rounded">👁️</button>
                      <button onClick={() => void navigate(`/tasks/${task.id}/edit`)} className="text-green-600 hover:bg-green-50 p-1 rounded">✏️</button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="flex justify-between items-center text-sm text-gray-600">
          <span>
            Hiện thị {indexOfFirstTask + 1} - {Math.min(indexOfLastTask, filteredTasks.length)} trong tổng số {filteredTasks.length} task nghiên cứu
          </span>
          <div className="flex gap-2">
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(number => (
              <button
                ...
                className={`px-3 py-1 rounded-lg ${
                  currentPage === number ? 'bg-green-700 text-white' : 'bg-gray-200 hover:bg-gray-300'
                }`}
              >
                {number}
              </button>
