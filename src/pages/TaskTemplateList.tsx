...
const TaskTemplateList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTemplates = async () => {
      ...
      try {
        const response = await axiosInstance.get('/api/tasktemplate?pageNumber=1&pageSize=100');
  ...
  const stageNames = useMemo(() => {
    const stages = [...new Set(templates.map(template => template.stageName))];
    return stages.sort();
  }, [templates]);
  ...
  const filteredAndSortedTemplates = useMemo(() => {
    ...
    if (stageFilter !== "Tất cả") {
      filteredTemplates = templates.filter(template => template.stageName === stageFilter);
    }
      ...
      <div className="max-w-4xl mx-auto bg-white rounded shadow p-8">
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            ...
            onClick={() => { void navigate("/task-templates/new"); }}
          >
            + Tạo mẫu nhiệm vụ
          </button>
        </div>
        ...
        <div className="mb-6 p-4 bg-gray-50 rounded-lg">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-700 font-medium">Lọc theo giai đoạn:</span>
              <select
                value={stageFilter}
        ...
        {loading ? (
          <div className="flex items-center justify-center py-8">
            <div className="text-gray-500">Đang tải danh sách mẫu nhiệm vụ...</div>
          </div>
        ) : error ? (
              ...
              {filteredAndSortedTemplates.length === 0 ? (
                ...
                filteredAndSortedTemplates.map((template) => (
                  <tr
                    ...
                    onClick={() => void navigate(`/task-templates/${template.id}`)}
                  >
                    <td className="py-3 px-4">{template.name}</td>
                    <td className="px-4">{template.stageName}</td>
                    <td className="px-4 text-gray-600">{template.description}</td>
                  </tr>
                ))
              )}
            </tbody>
