...
const CreateExperimentStep3 = () => {
  ...
  const handleCreateLog = () => {
    ...
    alert('Experiment Log đã được tạo!');
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-lg shadow">
          <div className="p-6 border-b">
            ...
            <p className="text-gray-600 mt-1">Bước 3: Review thông tin và hoàn thành</p>
          </div>
          ...
          <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <div className="bg-gray-50 p-4 rounded-lg">
                <h3 className="font-medium text-gray-900 mb-2">Tissue Culture Batch</h3>
                <p className="text-sm text-gray-700">{experimentData.batch.name}</p>
              </div>
              <div className="bg-gray-50 p-4 rounded-lg">
                <h3 className="font-medium text-gray-900 mb-2">Method</h3>
                <p className="text-sm text-gray-700">{experimentData.method.name}</p>
              </div>
              ...
              {experimentData.fatherPlant && (
                <div className="bg-blue-50 p-4 rounded-lg">
                  ...
                  <p className="text-sm text-blue-700">{experimentData.fatherPlant.name}</p>
                </div>
              )}
              {experimentData.motherPlant && (
                <div className="bg-pink-50 p-4 rounded-lg">
                  <h3 className="font-medium text-pink-800 mb-2">Cây Mẹ</h3>
                  <p className="text-sm text-pink-700">{experimentData.motherPlant.name}</p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-4">
              <div className="bg-green-50 p-4 rounded-lg">
                <h3 className="font-medium text-green-800 mb-2">Tóm Tắt Thông Tin</h3>
                <div className="text-sm text-green-700 space-y-1">
                  <div><strong>Batch:</strong> {experimentData.batch.name}</div>
                  <div><strong>Method:</strong> {experimentData.method.name}</div>
                  <div><strong>Lai tạo:</strong> {experimentData.fatherPlant?.name} x {experimentData.motherPlant?.name}</div>
                  <div><strong>Samples:</strong> {experimentData.samples} mẫu</div>
                  <div><strong>Ngày:</strong> {experimentData.createdDate}</div>
                </div>
              </div>

              <div className="bg-yellow-50 p-4 rounded-lg">
                <h3 className="font-medium text-yellow-800 mb-2">Dự kiến Kết Quả</h3>
