...
function isBatch(item: unknown): item is Batch {
  return typeof item === 'object' && item !== null && 'id' in item && typeof (item as { id: unknown }).id === 'string' && 'name' in item && typeof (item as { name: unknown }).name === 'string';
}

function isMethod(item: unknown): item is { id: string; name: string; description: string; type?: string; stages?: { id: string; name: string; description: string; dateOfProcessing: number; step: number; status: boolean }[] } {
  return (
...
const CreateExperimentStep1 = () => {
  ...
  const [experimentLogs, setExperimentLogs] = useState<{ id: string; tissueCultureBatchID: string; status: number | string }[]>([]);
  const [loadingEL, setLoadingEL] = useState(true);

  const [methods, setMethods] = useState<{ id: string; name: string; description: string; type?: string; stages?: { id: string; name: string; description: string; dateOfProcessing: number; step: number; status: boolean }[] }[]>([]);
  ...
  useEffect(() => {
    setLoadingEL(true);
    fetch('https://net-api.orchid-lab.systems/api/experimentlog?pageNumber=1&pageSize=1000')
      .then(async (res) => {
        if (!res.ok) throw new Error('Lỗi khi lấy danh sách experiment logs');
        const data: unknown = await res.json();
        let arr: { id: string; tissueCultureBatchID: string; status: number | string }[] = [];
        if (hasValueWithData<{ id: string; tissueCultureBatchID: string; status: number | string }>(data, (item): item is { id: string; tissueCultureBatchID: string; status: number | string } => {
          return typeof item === 'object' && item !== null && 
                 'id' in item && typeof (item as { id: unknown }).id === 'string' &&
                 'tissueCultureBatchID' in item && typeof (item as { tissueCultureBatchID: unknown }).tissueCultureBatchID === 'string' &&
                 'status' in item;
        })) {
          arr = data.value.data;
        }
  ...
  const isBatchAvailable = (batchId: string): { available: boolean; reason?: string } => {
    const relatedELs = experimentLogs.filter(el => el.tissueCultureBatchID === batchId);
    ...
    const statusText = Object.entries(statusCounts)
      .map(([status, count]) => {
        switch (status) {
          case "1": return `${count} thí nghiệm đã tạo`;
          case "2": return `${count} thí nghiệm đang thực hiện`;
          case "3": return `${count} thí nghiệm hoàn thành`;
          default: return `${count} thí nghiệm với trạng thái ${status}`;
        }
      })
      .join(", ");
          ...
          <div className="bg-white rounded-lg shadow">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                  ...
                  <div>
                      ...
                      >
                        ...
                        {batches.map((batch) => {
                          ...
                          return (
                            ...
                            >
                              {batch.name || batch.id} {!validation.available ? '(Đang sử dụng)' : ''}
                            </option>
                          );
                        })}
                      </select>
                    ...
                    {(loadingBatch || loadingEL) && <div className="text-xs text-gray-400 mt-1">Đang tải danh sách batch...</div>}
                    {batchError && <div className="text-xs text-red-500 mt-1">{batchError}</div>}
                    {selectedBatch && !loadingEL && (
                      (() => {
                        const validation = isBatchAvailable(selectedBatch);
                        if (validation.available) {
                          return <div className="text-xs text-green-600 mt-1">✓ Lô cấy mô này có thể sử dụng</div>;
                        } else {
                          return <div className="text-xs text-red-500 mt-1">✗ {validation.reason}</div>;
                        }
                      })()
                    )}
                  </div>
                      ...
                      <div className="space-y-2 text-sm text-gray-600">
                        ...
                        <div>
                          ...
                          <ul className="list-disc list-inside ml-4 space-y-1">
                            {methods.find(m => String(m.id) === selectedMethod)?.stages?.map((stage: { id: string; name: string; description: string; dateOfProcessing: number; step: number; status: boolean }, index: number) => (
                              <li key={stage.id}>
                  ...
                  {selectedBatch && batches.find(b => b.id === selectedBatch) && (
                    <div className="bg-orange-50 p-4 rounded-lg">
                      ...
                      <div className="text-sm text-orange-700 space-y-1">
                        <div>
                          <strong>Tên:</strong> {batches.find(b => b.id === selectedBatch)?.name ?? '---'}
                        </div>
                        <div>
                          <strong>Phòng thí nghiệm:</strong> {batches.find(b => b.id === selectedBatch)?.labName ?? '---'}
                        </div>
                        <div>
                          <strong>Mô tả:</strong> {batches.find(b => b.id === selectedBatch)?.description ?? '---'}
                        </div>
                      </div>
                    </div>
                  )}
            ...
            <div className="px-6 py-4 bg-gray-50 border-t flex justify-between">
              ...
              <div className="flex gap-4">
                              <button
                onClick={handleNext}
                disabled={!selectedBatch || !selectedMethod || (selectedBatch ? !isBatchAvailable(selectedBatch).available : false)}
                className={`px-6 py-2 rounded-lg font-medium transition-colors flex items-center gap-2 ${
                  !selectedBatch || !selectedMethod || (selectedBatch ? !isBatchAvailable(selectedBatch).available : false)
