...
const mockPlants: Plant[] = [
  { id: '015ac088-432e-429c-9714-ae47e599cd2c', name: 'Seedling-4', type: 'mẹ' },
  { id: '051b7baf-8b01-4e7d-a43f-ae27dcbd006c', name: 'TEST 3', type: 'mẹ' },
  { id: '0bcfc802-d32c-47be-876d-5c24b48cc0c5', name: 'Seedling-1', type: 'mẹ' },
  { id: '0ca0f52d-bcfe-4771-90ca-247b1c911e74', name: 'Seedling-9', type: 'cha' },
  { id: '0e5ccf9f-6b54-4f82-812f-d5829b13276b', name: 'Seedling-8', type: 'cha' },
  { id: '13936d9e-87e1-4ef4-8718-dd33a15d0214', name: 'Seedling-2', type: 'cha' },
];
...
const CreateExperimentStep2 = () => {
  ...
  // Điều kiện next
  const isNextDisabled = methodName === 'Subculturing'
    ? !selectedMother
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                {methodName === 'Subculturing' && (
                  <div>
                    <h3 className="text-lg font-semibold text-gray-800 mb-3">Chọn 1 cây mẹ</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {mockPlants.filter(p => p.type === 'mẹ').map((plant) => (
                        <div
                          key={plant.id}
                          className={`border-2 rounded-lg p-4 cursor-pointer flex items-center gap-4 transition-all ${selectedMother?.id === plant.id ? 'border-green-600 bg-green-50' : 'border-gray-200 hover:border-gray-300'}`}
                          onClick={() => setSelectedMother(plant)}
                        >
                          <div className={`w-5 h-5 rounded-sm border-2 flex items-center justify-center ${selectedMother?.id === plant.id ? 'bg-green-600 border-green-600' : 'border-gray-300'}`}>
                            {selectedMother?.id === plant.id && <Check className="w-4 h-4 text-white" />}
                          </div>
                          <div>
                ...
                {methodName === 'Sterilization' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <h3 className="text-lg font-semibold text-gray-800 mb-3">Chọn 1 cây cha</h3>
                      <div className="grid grid-cols-1 gap-4">
                        {mockPlants.filter(p => p.type === 'cha').map((plant) => (
                          <div
                            key={plant.id}
                            className={`border-2 rounded-lg p-4 cursor-pointer flex items-center gap-4 transition-all ${selectedFather?.id === plant.id ? 'border-green-600 bg-green-50' : 'border-gray-200 hover:border-gray-300'}`}
                            onClick={() => setSelectedFather(plant)}
                          >
                            <div className={`w-5 h-5 rounded-sm border-2 flex items-center justify-center ${selectedFather?.id === plant.id ? 'bg-green-600 border-green-600' : 'border-gray-300'}`}>
                              {selectedFather?.id === plant.id && <Check className="w-4 h-4 text-white" />}
                            </div>
                            <div>
                              <div className="font-medium">{plant.name}</div>
                            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-orange-50 p-4 rounded-lg">
                  ...
                  <div className="text-sm text-orange-700 space-y-1">
                    ...
                    {methodName === 'Sterilization' && (
                      <>
                        <div><strong>Cha:</strong> {selectedFather?.name ?? 'Chưa chọn'}</div>
                        <div><strong>Mẹ:</strong> {selectedMother?.name ?? 'Chưa chọn'}</div>
                      </>
                    )}
                  </div>
                </div>
