...
export default function AdminTasks() {
  ...
  const [completedOnTimeMode, setCompletedOnTimeMode] = useState<"day" | "week" | "month">("day");
    ...
    const loadData = async () => {
      try {
        ...
        if (isApiTaskResponse(response.data)) {
          const all = Array.isArray(response.data.value?.data) ? response.data.value.data : [];
          setAllTasks(all);
  ...
  useEffect(() => {
    ...
    if (researcherFilter !== "Tất cả") {
      filteredData = filteredData.filter(t => t.researcher === researcherFilter);
    }
    if (searchTerm.trim()) {
  ...
  const completedOnTimeChartStats = useMemo(() => {
    ...
    const completedTasks = allTasks.filter(task => task.status === "DoneInTime");
    ...
    <main className="ml-64 mt-16 min-h-[calc(100vh-64px)] bg-gray-50 p-8">
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-bold text-gray-900">Thống kê tổng nhiệm vụ được tạo</h1>
          <select
            value={timeMode}
        ...
        <div className="bg-white rounded-lg shadow p-6 space-y-4">
          <div className="flex items-center gap-4">
            <h1 className="text-2xl font-bold text-gray-900">Thống kê nhiệm vụ được tạo cụ thể</h1>
            <select
              value={filterMode}
        ...
        <div className="bg-white rounded-lg shadow p-6 space-y-4">
          <div className="flex items-center gap-4">
            <h1 className="text-2xl font-bold text-gray-900">Thống kê nhiệm vụ hoàn thành đúng hạn</h1>
            <select
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-6 mb-8">
          {Object.entries(STATUS_SUMMARY_LABELS).map(([key, label]) => (
            <div key={key} className="rounded-lg border border-gray-200 bg-white px-6 py-4 flex flex-col items-center">
              <span className="text-sm text-gray-600 mb-1">{label}</span>
              <span className={`text-2xl font-semibold ${STATUS_COLORS[key as StatusType]}`}>
