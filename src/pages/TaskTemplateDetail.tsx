...
const TaskTemplateDetail: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    axiosInstance.get(`/api/tasktemplate/${id}`)
      ...
      .catch(() => {
        setError(true);
        enqueueSnackbar('Không thể tải chi tiết mẫu nhiệm vụ!', { variant: 'error' });
      })
      .finally(() => {
        setLoading(false);
      });
    ...
    <main className="ml-64 mt-16 min-h-[calc(100vh-64px)] bg-gray-100 flex flex-col items-center py-10 px-4">
      <div className="bg-white rounded-xl px-8 pt-8 pb-8 shadow-[0_2px_8px_rgba(0,0,0,0.06)] w-full max-w-[900px] mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold">Chi tiết mẫu nhiệm vụ: {template.name}</h2>
        ...
        <div className="flex flex-col mb-6">
          ...
          <span className={`px-3 py-2 rounded-md text-sm font-medium w-fit ${template.status ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
          ...
          {template.details && template.details.length > 0 ? (
            <div className="space-y-2">
              ...
              {template.details.map((detail, idx) => (
                <div className="grid grid-cols-1 md:grid-cols-5 gap-3 mb-2" key={detail.id || idx}>
                  <div className="px-3 py-2 border border-gray-300 rounded-md bg-gray-50 text-gray-700">
                  ...
                  <div className="px-3 py-2 border border-gray-300 rounded-md bg-gray-50 text-gray-700">
                    <span className={`px-2 py-1 rounded-full text-xs ${detail.isRequired ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
