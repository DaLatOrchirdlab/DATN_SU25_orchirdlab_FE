...
const TaskTemplateCreate: React.FC = () => {
  ...
  useEffect(() => {
    ...
    axiosInstance.get("/api/method?pageNumber=1&pageSize=100")
      .then(res => {
      ...
      .catch(() => {
        setMethods([]);
        enqueueSnackbar('Không thể tải danh sách phương pháp!', { variant: 'error' });
      })
  ...
  const handleDetailChange = (idx: number, field: keyof TemplateDetail, value: string | number | boolean) => {
  ...
  if (details.some(d => !d.element || d.expectedValue <= 0)) {
      enqueueSnackbar('Vui lòng điền đầy đủ thông tin nguyên vật liệu!', { variant: 'error' });
      return;
    }
        ...
        <form onSubmit={(e) => { void handleSubmit(e); }}>
          ...
          <div className="mb-4">
            ...
            <select
              ...
              onChange={e => { setSelectedMethodId(e.target.value); setSelectedStageId(""); }}
              required
              className="w-full py-2 px-3 border border-gray-300 rounded-md text-base bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-500"
            >
            ...
            {loadingMethods && <span className="text-xs text-gray-400">Đang tải...</span>}
          </div>
            ...
            <div className="space-y-4">
              {details.map((detail, idx) => (
                <div key={idx} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                  ...
                  <div className="mb-3">
                    <label className="text-sm font-medium text-gray-700 mb-1 block">Nguyên vật liệu *</label>
                    <select
                      value={detail.element}
                      onChange={e => handleDetailChange(idx, "element", e.target.value)}
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    ...
                    <input
                      ...
                      onChange={e => handleDetailChange(idx, "expectedValue", Number(e.target.value))}
                      className="w-full py-2 px-3 border border-gray-300 rounded-md text-base bg-white focus:outline-none focus:ring-2 focus:ring-green-500"
                      required
                    />
                    <input
