...
interface LabRoom { id: string; name: string }
...
const AdminTissueCultureBatchDetail = () => {
  ...
  useEffect(() => {
    if (!id) return;
    setLoading(true);
    axiosInstance.get(`/api/tissue-culture-batch/${id}`)
      .then(res => {
  ...
  useEffect(() => {
    axiosInstance.get('/api/labroom?pageNumber=1&pageSize=100')
      .then(res => {
        ...
        const arr = (raw as any)?.value?.data ?? (Array.isArray(raw) ? raw : []);
        setLabRooms(arr);
      })
      .catch(() => setLabRooms([]));
  }, []);
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            {!editing && (
              <button className="px-4 py-2 rounded bg-gray-600 text-white" onClick={() => setEditing(true)}>Cập nhật</button>
            )}
            <button className="px-4 py-2 rounded bg-red-600 text-white" onClick={handleDelete}>Xóa</button>
          </div>
        </div>
        ...
        {!editing ? (
          <div className="space-y-2">
            ...
            <p><b>Phòng thí nghiệm:</b> {data.labName ?? '-'}</p>
            <p><b>Mô tả:</b> {data.description ?? '-'}</p>
            <p><b>Đang sử dụng trong:</b> {data.inUse ?? '-'}</p>
            <p><b>Trạng thái:</b> {data.status ? 'Còn hoạt động' : 'Ngừng hoạt động'}</p>
          </div>
        ) : (
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium mb-1">Phòng thí nghiệm mới</label>
              <select className="w-full border rounded px-3 py-2" value={selectedLab} onChange={e => setSelectedLab(e.target.value)}>
                <option value="">Giữ nguyên</option>
                {labRooms.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}
              </select>
            </div>
            <div>
            ...
            <div className="flex justify-end gap-2">
              <button className="px-4 py-2 rounded border" onClick={() => setEditing(false)}>Hủy</button>
              <button className="px-4 py-2 rounded bg-green-600 text-white" onClick={handleSave} disabled={saving}>{saving ? 'Đang lưu...' : 'Hoàn thành'}</button>
            </div>
          </div>
        )}
      </div>
