...
const AutoCreateTaskContainer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    axiosInstance.get('/api/user?pageNumber=1&pageSize=100')
      .then(res => {
        ...
        const filteredTechnicians = data.filter(t => String(t.roleID) === "3");
  ...
  const handleCreateTasks = async () => {
    ...
    if (!experimentLogId || !stageId) {
      enqueueSnackbar('Thiếu thông tin nhật ký thí nghiệm hoặc giai đoạn', { variant: 'error' });
      return;
    }
    ...
    try {
      ...
      enqueueSnackbar(`Đã tạo thành công ${results.length} nhiệm vụ!`, { variant: 'success' });
      void navigate('/tasks');
    } catch (error: unknown) {
      ...
      if (error && typeof error === 'object' && 'response' in error && 
        ...
        enqueueSnackbar(`Lỗi khi tạo nhiệm vụ: ${(error.response.data as { message: string }).message}`, { variant: 'error' });
      } else {
    ...
    return (
      <main className="ml-64 mt-16 min-h-[calc(100vh-64px)] bg-gray-100 flex flex-col items-center py-10">
        <div className="bg-white rounded-xl px-8 pt-8 pb-6 shadow-md max-w-2xl w-full text-center">
          <h2 className="text-2xl font-bold mb-6 text-red-700">Truy cập không hợp lệ</h2>
          <p className="text-gray-600 mb-4">Vui lòng truy cập từ trang chi tiết nhật ký thí nghiệm</p>
          <button
  ...
  if (loadingTemplates || loadingTechnicians || loadingMethod) {
    return (
      <main className="ml-64 mt-16 min-h-[calc(100vh-64px)] bg-gray-100 flex flex-col items-center py-10">
        <div className="bg-white rounded-xl px-8 pt-8 pb-6 shadow-md max-w-2xl w-full text-center">
          <h2 className="text-2xl font-bold mb-6 text-green-800">Đang tải...</h2>
    ...
    <main className="ml-64 mt-16 min-h-[calc(100vh-64px)] bg-gray-100 flex flex-col items-center py-10">
      <div className="bg-white rounded-xl px-8 pt-8 pb-8 shadow-[0_2px_8px_rgba(0,0,0,0.06)] w-full max-w-4xl mx-auto">
        ...
        <div className="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
          <h3 className="font-semibold text-blue-800 mb-2">Thông tin tự động</h3>
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <span className="font-medium">Phương pháp:</span> {method?.name ?? 'Đang tải...'}
            </div>
            <div>
        ...
        <div className="mb-6">
          <h3 className="font-semibold mb-4">Mẫu nhiệm vụ sẽ được tạo ({taskTemplates.length} mẫu)</h3>
              ...
              <div key={template.id} className="p-4 border border-gray-200 rounded-lg bg-gray-50">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-gray-600 text-sm">Giai đoạn: {template.stageName}</p>
                 ...
                 {template.details && template.details.length > 0 && (
                   <div className="mt-3">
                     <p className="text-sm font-medium text-gray-700 mb-2">Nguyên vật liệu:</p>
                       ...
                       {template.details.map((detail, idx) => (
                         <div key={idx} className="text-xs bg-white p-3 rounded border">
                           ...
                           <div className="text-gray-600 text-xs mt-1">
                             <span className="font-medium">Số lượng:</span> {detail.expectedValue} {detail.unit}
                             
                           </div>
                           <div className="text-gray-600 text-xs mt-1">
                           <span className="font-medium">Đơn vị:</span>  {detail.unit}
                           </div>
                           {detail.description && (
                             <div className="text-gray-500 text-xs mt-1">
                               <span className="font-medium">Mô tả:</span> {detail.description}
                             </div>
                           )}
                         </div>
                       ))}
        ...
        <div className="mb-6">
          ...
          {loadingTechnicians && <span className="text-xs text-gray-400">Đang tải...</span>}
        </div>
