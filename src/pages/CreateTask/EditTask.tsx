...
const EditTask: React.FC = () => {
  ...
  const [taskData, setTaskData] = useState<TaskData>({
    ...
    method: "Nuôi cấy mô",
    description:
      "Thực hiện lai tạo giữa các dòng P. amabilis để tạo ra giống mới có độ bền cao. Nghiên cứu khả năng sinh trưởng và kháng bệnh.",
    tasks: [
    ...
    selectedTech: {
      id: 1,
      name: "Trần Văn Kang",
      info: "Kỹ thuật viên chính - 5 năm",
    },
  });
    ...
    <main className="ml-64 mt-16 min-h-[calc(100vh-64px)] bg-gray-100 flex flex-col items-center py-10 px-4">
      ...
      >
        <h2 className="text-2xl font-semibold mb-6">
          Chỉnh sửa nhiệm vụ: {taskData.id}
        </h2>
          ...
          {taskData.tasks.map((task, idx) => (
            ...
            >
              <input
                ...
                value={task.name}
                onChange={(e) =>
                  handleTaskDetailChange(idx, "name", e.target.value)
                }
              ...
              <input
                ...
                value={task.value}
                onChange={(e) =>
                  handleTaskDetailChange(idx, "value", e.target.value)
                }
                placeholder="Giá trị"
                className="px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
              />
              <div className="flex gap-2">
                <input
                  ...
                  value={task.unit}
                  onChange={(e) =>
                    handleTaskDetailChange(idx, "unit", e.target.value)
                  }
                  placeholder="Đơn vị"
        ...
        <div className="flex flex-col mb-6">
          <label className="font-medium mb-1.5">
            Lồng đã chọn (không thể thay đổi)
          </label>
        ...
        <div className="flex flex-col mb-8">
          <label className="font-medium mb-1.5">
            Kỹ thuật viên được giao (không thể thay đổi)
          </label>
          <div className="bg-green-50 border-[1.5px] border-green-700 rounded-lg py-2.5 px-[18px] font-semibold flex items-center mt-1 min-h-[40px]">
